...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`pages_rels\` (
  	...
  	FOREIGN KEY (\`recommendations_id\`) REFERENCES \`recommendations\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`contributions_id\`) REFERENCES \`contributions\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`friends_id\`) REFERENCES \`friends\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`pages_rels_recommendations_id_idx\` ON \`pages_rels\` (\`recommendations_id\`);`)
  await db.run(sql`CREATE INDEX \`pages_rels_contributions_id_idx\` ON \`pages_rels\` (\`contributions_id\`);`)
  await db.run(sql`CREATE INDEX \`pages_rels_friends_id_idx\` ON \`pages_rels\` (\`friends_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_pages_v_rels_recommendations_id_idx\` ON \`_pages_v_rels\` (\`recommendations_id\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_rels_contributions_id_idx\` ON \`_pages_v_rels\` (\`contributions_id\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_rels_friends_id_idx\` ON \`_pages_v_rels\` (\`friends_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`recommendations\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`name\` text,
  	\`profile_url\` text,
  	\`headline\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`recommendations_image_idx\` ON \`recommendations\` (\`image_id\`);`)
  await db.run(sql`CREATE INDEX \`recommendations_updated_at_idx\` ON \`recommendations\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`recommendations_created_at_idx\` ON \`recommendations\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`recommendations__status_idx\` ON \`recommendations\` (\`_status\`);`)
  await db.run(sql`CREATE TABLE \`_recommendations_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_name\` text,
  	\`version_profile_url\` text,
  	\`version_headline\` text,
  	\`version_relationship\` text,
  	\`version_text\` text,
  	...
  	\`latest\` integer,
  	\`autosave\` integer,
  	FOREIGN KEY (\`parent_id\`) REFERENCES \`recommendations\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`_recommendations_v_parent_idx\` ON \`_recommendations_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_recommendations_v_version_version_image_idx\` ON \`_recommendations_v\` (\`version_image_id\`);`)
  await db.run(sql`CREATE INDEX \`_recommendations_v_version_version_updated_at_idx\` ON \`_recommendations_v\` (\`version_updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_recommendations_v_version_version_created_at_idx\` ON \`_recommendations_v\` (\`version_created_at\`);`)
  await db.run(sql`CREATE INDEX \`_recommendations_v_version_version__status_idx\` ON \`_recommendations_v\` (\`version__status\`);`)
  await db.run(sql`CREATE INDEX \`_recommendations_v_created_at_idx\` ON \`_recommendations_v\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`_recommendations_v_updated_at_idx\` ON \`_recommendations_v\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_recommendations_v_latest_idx\` ON \`_recommendations_v\` (\`latest\`);`)
  await db.run(sql`CREATE INDEX \`_recommendations_v_autosave_idx\` ON \`_recommendations_v\` (\`autosave\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`contributions_image_idx\` ON \`contributions\` (\`image_id\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`contributions_slug_idx\` ON \`contributions\` (\`slug\`);`)
  ...
  await db.run(sql`CREATE INDEX \`contributions__status_idx\` ON \`contributions\` (\`_status\`);`)
  await db.run(sql`CREATE TABLE \`_contributions_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_title\` text,
  	...
  	\`latest\` integer,
  	\`autosave\` integer,
  	FOREIGN KEY (\`parent_id\`) REFERENCES \`contributions\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`_contributions_v_parent_idx\` ON \`_contributions_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_contributions_v_version_version_image_idx\` ON \`_contributions_v\` (\`version_image_id\`);`)
  await db.run(sql`CREATE INDEX \`_contributions_v_version_version_slug_idx\` ON \`_contributions_v\` (\`version_slug\`);`)
  await db.run(sql`CREATE INDEX \`_contributions_v_version_version_updated_at_idx\` ON \`_contributions_v\` (\`version_updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_contributions_v_version_version_created_at_idx\` ON \`_contributions_v\` (\`version_created_at\`);`)
  await db.run(sql`CREATE INDEX \`_contributions_v_version_version__status_idx\` ON \`_contributions_v\` (\`version__status\`);`)
  await db.run(sql`CREATE INDEX \`_contributions_v_created_at_idx\` ON \`_contributions_v\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`_contributions_v_updated_at_idx\` ON \`_contributions_v\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_contributions_v_latest_idx\` ON \`_contributions_v\` (\`latest\`);`)
  await db.run(sql`CREATE INDEX \`_contributions_v_autosave_idx\` ON \`_contributions_v\` (\`autosave\`);`)
  await db.run(sql`CREATE TABLE \`friends\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`name\` text,
  	\`url\` text,
  	\`description\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`friends_image_idx\` ON \`friends\` (\`image_id\`);`)
  await db.run(sql`CREATE INDEX \`friends_updated_at_idx\` ON \`friends\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`friends_created_at_idx\` ON \`friends\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`friends__status_idx\` ON \`friends\` (\`_status\`);`)
  await db.run(sql`CREATE TABLE \`_friends_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_name\` text,
  	\`version_url\` text,
  	...
  	\`latest\` integer,
  	\`autosave\` integer,
  	FOREIGN KEY (\`parent_id\`) REFERENCES \`friends\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`_friends_v_parent_idx\` ON \`_friends_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_friends_v_version_version_image_idx\` ON \`_friends_v\` (\`version_image_id\`);`)
  await db.run(sql`CREATE INDEX \`_friends_v_version_version_updated_at_idx\` ON \`_friends_v\` (\`version_updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_friends_v_version_version_created_at_idx\` ON \`_friends_v\` (\`version_created_at\`);`)
  await db.run(sql`CREATE INDEX \`_friends_v_version_version__status_idx\` ON \`_friends_v\` (\`version__status\`);`)
  await db.run(sql`CREATE INDEX \`_friends_v_created_at_idx\` ON \`_friends_v\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`_friends_v_updated_at_idx\` ON \`_friends_v\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_friends_v_latest_idx\` ON \`_friends_v\` (\`latest\`);`)
  await db.run(sql`CREATE INDEX \`_friends_v_autosave_idx\` ON \`_friends_v\` (\`autosave\`);`)
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_recommendations_id_idx\` ON \`payload_locked_documents_rels\` (\`recommendations_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_friends_id_idx\` ON \`payload_locked_documents_rels\` (\`friends_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`recommendations\`;`)
  await db.run(sql`DROP TABLE \`_recommendations_v\`;`)
  await db.run(sql`DROP TABLE \`contributions\`;`)
  await db.run(sql`DROP TABLE \`_contributions_v\`;`)
  await db.run(sql`DROP TABLE \`friends\`;`)
  await db.run(sql`DROP TABLE \`_friends_v\`;`)
